...
function MyComponent({xstyle}) {
  return <div className={stylex(styles.foo, styles.bar, xstyle)} />;
}
```
...
because of how fast the `stylex()` and `stylex.spread()` functions are.
...
themselves. This also makes testing easier as we can remove the styles entirely
in a testing environment to remove the noise caused by constantly changing
styles.
...
Other functions within the StyleX API should be simple utilities whose purpose
should be self-evident by their names.
...
StyleX is a CSS-in-JS solution, not a CSS-in-React solution. Althought StyleX
works best with React today, It's designed to be used with any Javascript
framework that lets you author your markup in your Javacript.

Some APIs, such as `stylex.spread` return an object with `className` and `style`
keys today. You may need to use a wrapper function to make this work with your
framework of choice.
...
We already have some ideas to make these patterns possible in the future,
without breaking the core principle of style encapsulation.
...
We want every codebase that uses StyleX to feel instantly familiar. You
shouldn't have to dig through configuration files to learn the conventions of a
project. Our goal is to remove all "magic strings" and replace them with
familiar patterns such as constants and imports.

Any project specific style restrictions should be enforced by one of the
following:

1. Exports from JS files.

2. ESlint Configuration: We do allow configuration of our ESLint rules to
   enforce project-specific conventions.
3. Static Types: You can create static types to add constraints to the styles
   components can accept.

This principle can be seen in practice in a few locations already:

- You will never have to remember a className. All styles are applied by passing
  object references to `stylex.spread` or `stylex()`.
- You don't have to remember names of CSS variables. You import and use them by
  reference.
- You don't have to remember the names of keyframes. You define them and use
  them by reference.

There are additional locations where CSS makes you use magic strings today, e.g.
`container-name`. We're looking into ways to replace these magic strings with
type-safe APIs as well.
