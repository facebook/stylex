...
Unless you opt into a feature, you should never pay for it.
...
CSS is a powerful and extremely expressive language. However, it also feels
extremely fragile. Some of this stems from a misunderstand how CSS works
(box-sizing: content-box, anyone?), but a lot of it stems from the fact that CSS
selectors have different specificity and the rules of precendence are complex.

Whenever we start applying multiple CSS rules (classes) to an element, we start
to lose control of what the final result will be. We start to bumping the
specificity of our CSS rules, and adding `!important` to our styles and the
whole thing becomes a game of whack-a-mole.

Many of the most successful solutions this problem involve following rules
through discipline. BEM introduces a naming convention such that we don't run
into many of these problems. But it puts the burden on the developer to follow
the rules and it mostly just tells you not to merge styles at all. This leads to
bloated CSS.

Atomic utility classNames like Tailwind CSS and Tachyons solve this problem by
putting the onus on the developer to not put multiple utility classes that sets
the same style on the same element. It also doesn't let you merge styles from
...
TLDR; We want to make this meme a thing of the past:

![CSS Blinds](/img/css-blinds.gif)

## Readability & Maintainabity over Terseness

Many recent styling solutions are extremely terse. Tailwind CSS is popular and
lets you author your styles by putting extremely terse utility classNames
directly on your markup. This is great for authoring styles quickly, but in our
opinion, in the long run it makes the styles harder to read and maintain.

These systems also introduce a completely new naming pattern that developers
have to learn and internalize.

StyleX makes the choice to use the same CSS property names that you're used to.
(We did decide to use camelCase instead of kebab-case as that makes them easier
to type in JavaScript.) This makes it easier to learn and read.
...
## Styles should be Type-Safe too
...
All styles are typed. When accepting styles as props, you can use types to
