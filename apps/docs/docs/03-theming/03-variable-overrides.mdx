...
"providers" for them using the `stylex.varsProvider` function. The providers
...
defined using the `stylex.createVars` function. Similar to React's Context API.
...
Unlike when you define variables, your can create these `varsProvider`s anywhere
