...
StyleX uses atomic styles, which means that each CSS class only contains a single rule, and each rule is contained in only one CSS class. For example, if we look at the example from earlier:
...
Three CSS classes will be generated: One for the `width: '100%'` rule, one for the `maxWidth: 800`, and one for the `minHeight: 40`. If you use `width: '100%'` anywhere else in your application, it will *reuse the same CSS class* rather than creating a new one.

This means that your CSS file will grow *logarithmically* as you add new components to your app, since the styles needed by new components will likely already exist in the CSS file. As a result, the CSS file is usually small enough that it can all be bundled as a single bundle rather than splitting it per route, avoiding style recalculation and layout thrashing as users navigate through your app.
