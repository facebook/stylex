...
set the appropriate `className` prop on the HTML element and, when using
...
StyleX supports this pattern without additional API. Instead, object property
lookup can be used to achieve the same result. First, each variant can defined
with the appropriate variant name for the style object.
...
Multiple style style objects, including conditional styles, can be passed in
...
Accepting custom `StyleX Styles` is as simple as accepting any other prop. and
