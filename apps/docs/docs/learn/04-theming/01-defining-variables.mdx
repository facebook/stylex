...
Once the variables have been created, you can create "provider" to overridde the
values for particular sub-trees within your UI.

## Defining Variables

You define variables by using the `stylex.defineVars` function:
...
Each of the keys in the object passed to `stylex.defineVars` points to a
...
```
You can use `@supports` similarly.
...
Every `stylex.defineVars` call _must_ be part of a named export.
...
StyleX _could_ work without the need for these strict rules. However, by
enforcing these rules we can build a build pipeline with strict file boundaries
where we can confidently cache the results of the compilation process on a
per-file basis.

With better and faster tooling, we remove these restrictions in the future.
