...
Once you have defined your variables in a `.stylex.js` file, you can alternate
themes for them with different values using the `stylex.createTheme` function. 
The providers created are the same type as the `StyleXStyles` type that is returned 
from `stylex.create` and can be be used on any UI sub-tree in your app.
...
To override a one-off variable, you will soon be able to use variables as *keys*
while defining styles with `stylex.create`.

:::

The value returned from `stylex.createTheme` is of the `StyleXStyles` type that
