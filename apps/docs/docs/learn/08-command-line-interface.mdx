...
StyleX has a CLI that can be used as an escape hatch in build systems that are not yet supported by StyleX. The CLI can be used standalone to compile your StyleX styles. 
The CLI internally is a simple babel wrapper that uses the StyleX babel plugin, and is intended to be run prior to your build system.
...
You can create a `.json` or `.json5` config file and pass it to the CLI via the `--config` flag.
...
Name of the StyleX output directories. The order of the output directories should match the order of the input directories.
