...
export default function styleXDefineVars<Vars: VarsConfig>(
  ...
  const variablesMap: {
    ...
    let explicitCustomPropertyName = null;
    if (isNamedVarSpec(value)) {
      if (!value.name.startsWith('--')) {
        throw new Error(messages.namedVarNameMustStartWithDashes(key, value.name));
      }
      if (!isValidCustomPropertyName(value.name)) {
        throw new Error(messages.namedVarInvalidCustomPropertyName(key, value.name));
      }
      explicitCustomPropertyName = value.name;
