...
/// This function looks for `stylex.unstable_createThemeNested` calls and transforms them.
/// 1. It finds the first two arguments to `stylex.unstable_createThemeNested` and validates those.
/// 2. This handles local constants automatically.
/// 4. It uses the `stylexCreateThemeNested` from `@stylexjs/shared` to transform the JS
...
export default function transformStyleXCreateThemeNested(
  ...
  if (
    (callExpressionNode.callee.type === 'Identifier' &&
      state.stylexCreateThemeNestedImport.has(
    ...
    validateStyleXCreateThemeNested(callExpressionPath);
...
// Validates the call of `stylex.unstable_createThemeNested`.
function validateStyleXCreateThemeNested(
  callExpressionPath: NodePath<t.CallExpression>,
) {
  ...
  if (
    ...
    throw callExpressionPath.buildCodeFrameError(
      messages.unboundCallValue('unstable_createThemeNested'),
      SyntaxError,
    );
  }
  ...
  if (callExpressionPath.node.arguments.length !== 2) {
    throw callExpressionPath.buildCodeFrameError(
      messages.illegalArgumentLength('unstable_createThemeNested', 2),
      SyntaxError,
    );
  }
}
