...
The only requirement for using the CLI is that your bundler is able to handle CSS imports,
as the CLI inserts an import for the generated CSS file into every file that was transformed.
...
Although all options for the CLI are available as command line arguments, using a configuration file is
recommended. The configuration file can be in either JSON or JSON5 format. Using JSON5 lets you
skip quotes around object keys, and use comments and trailing commas.
...
The configuration file can be named anything, as it is always passed explicitly to the CLI
