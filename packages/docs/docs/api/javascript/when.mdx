...
To use descendant and sibling selectors, you must mark the element being observed by passing the `stylex.defaultMarker()` class name.
...
Styles an element based on the state of a descendant element in the DOM tree. Relies on the CSS `:has()` selector.
...
Styles an element based on the state of a following sibling element. Relies on the CSS `:has()` selector.
...
Styles an element based on the state of any sibling element (before or after). Relies on the CSS `:has()` selector.
...
When multiple `stylex.when.*` selectors apply to the same element, they are ordered by specificity. The `sort-keys` ESLint rule enforces this order:

1. `ancestor` (highest specificity)
2. `descendant`
3. `anySibling`
4. `siblingBefore`
5. `siblingAfter` (lowest specificity)
