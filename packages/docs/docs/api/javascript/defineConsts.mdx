...
Defines static style constants that can be used directly in `stylex.create` calls anywhere in the codebase. Unlike defineVars, these values are inlined at build time and do not generate actual CSS variables.
...
export const zIndexes = stylex.defineConsts({
  modal: '1000',
  tooltip: '1100',
  toast: '1200',
});
...
```tsx
import * as stylex from '@stylexjs/stylex';
import { breakpoints, zIndexes, animations } from './constants.stylex.js';
