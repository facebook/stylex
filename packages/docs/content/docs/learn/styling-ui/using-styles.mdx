...
A simple way to think about the `props` function is that it merges many objects
and the later objects have precedence over previous objects.
...
## Inline atomic styles

For small one-off styles, `@stylexjs/atoms` can be used directly inside
`stylex.props` without creating a separate `stylex.create` namespace.
...
<div
  {...stylex.props(
    ...
    x.alignItems.center,
    x.gap._8px,
    isActive && x.color.blue,
  )}
/>;
```

Atoms are compiled away by the StyleX Babel plugin and produce the same output
as equivalent `stylex.create` styles. Use
[`@stylexjs/atoms`](/docs/api/javascript/atoms) for inline styles that are
easier to read at the callsite; use `stylex.create` when a set of styles is
reused or benefits from a descriptive local name.
