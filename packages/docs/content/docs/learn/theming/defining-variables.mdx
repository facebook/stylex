...
## Deriving variables from other variables

`defineVars` also supports zero-argument function values for derived variables.
These functions are evaluated at compile time and can reference other keys from
the same variable group.
...
export const colors = stylex.defineVars({
  ...
  textSubtle: () => `color-mix(${colors.textMuted}, transparent 50%)`,
});
```

This is useful when several theme values build on top of the same base token.
StyleX detects missing references and cyclical references at compile time.
