...
Function values are also supported for untyped variable leaves. These functions
run at compile time and can reference other keys from the same `defineVars`
group. The output remains a normal CSS variable reference.
