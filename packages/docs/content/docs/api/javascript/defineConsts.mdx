...
With `enableMediaQueryOrder` enabled, whole at-rule constants participate in
condition ordering when `processStylexRules` resolves their values. Use
`@stylexjs/lightningcss-plugin` after that step to simplify the resulting
negations.
