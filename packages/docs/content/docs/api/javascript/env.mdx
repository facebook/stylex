...
Define shareable functions using the `env` API. All function arguments must be locally defined. Functions
must be pure and return strings or numbers. They are called at compile time and their return
values are inlined.
