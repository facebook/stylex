...
Discovery reads the dependency names in the project's own `package.json` and
does not recurse. A package that contains StyleX but is reached only through
another dependency is never inspected.

Passing `include` disables auto-discovery entirely, along with the default
`exclude` patterns. Every dependency that ships uncompiled StyleX then has to be
listed, and the paths may point into `node_modules`:
...
Styles that are compiled by Babel but never scanned here produce class names
with no matching rules, without an error.
