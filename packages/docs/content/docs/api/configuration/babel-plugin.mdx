...
extraction. It also enables the DOM metadata used by the StyleX Dev Tools
Extension.
...
```ts
sxPropName?: string; // Default: 'sx'
```

Customize the prop name that should be used instead of `sx` on HTML JSX
elements.

#### Example

If you want to use `css` instead of `sx`:
...
```ts
sxOutput: 'as-props' | 'as-attrs'; // Default: 'as-props'
```

Controls how the `sx` JSX prop is compiled.

- **as-props**: rewrites `sx={styles.foo}` to `...stylex.props(styles.foo)`,
  producing `className`.
- **as-attrs**: rewrites `sx={styles.foo}` to `...stylex.attrs(styles.foo)`,
  producing `class`.

This only affects the JSX `sx` shortcut. Direct calls to `stylex.props()` and
`stylex.attrs()` keep their existing behavior.
...
Set this option to another string to use a different prop name, such as `css`.
Set it to `false` to disable the JSX shorthand entirely.
...
`processStylexRules` takes an array of CSS rules collected from the babel plugin
and returns the final CSS string. The optional `config` parameter controls layer
...
ordering. When `true`, each priority level is placed in its own layer. When your
project also uses other CSS layers (e.g. a reset layer, a design-system layer,
or a utility framework like Tailwind), pass an object to position StyleX's
layers relative to yours in the
[`@layer` ordering declaration](https://developer.mozilla.org/en-US/docs/Web/CSS/@layer)
and to namespace them to avoid collisions:

- **`before`** — layer names placed _before_ StyleX layers in the ordering
  header.
...
as **`useCSSLayers`** and pass it through to `processStylexRules` as
`useLayers`.
