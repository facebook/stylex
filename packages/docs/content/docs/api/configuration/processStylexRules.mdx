...
> **Note**: The `useLegacyClassnamesSort` option may not be included in the
> published TypeScript definitions for some versions of `@stylexjs/babel-plugin`.
> You can still use this option at runtime, but you may need to use a type
> assertion in your build setup until the official typings are updated.
